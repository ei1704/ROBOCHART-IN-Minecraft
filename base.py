...
def start():
    ...
    for i in range(3):
        for j in range(8):
            ...
            #sclipt[c]=mp[j][i]
            c+=1
            if mp[j][i]!=0 and mp[j][i].kind==5:
                judge+=1
                stack.append((j,i))                
            elif mp[j][i]!=0 and mp[j][i].kind==6:
                judge-=1
                if judge<0:
                    messagebox.showerror("Alert","ループの始まりが見つかりません。")
                    return
                else:
                    mp[j][i].sPoint=stack.pop()
                    temp=mp[j][i].sPoint
                    mp[j][i].number=mp[temp[0]][temp[1]].num
                    mp[temp[0]][temp[1]].gPoint=(j,i)
            
    if judge!=0:
        messagebox.showerror("Alert","ループが閉じられていません。")
        return      
    ...
    while i<3:
        while j<8:
          ...
          #        j=temp[1]
            ...
            temp=mp[j][i].sPoint
            if mp[j][i].number>1:
                mp[j][i].number=mp[j][i].number-1

                print(str(mp[j][i].number))
                temp=mp[j][i].sPoint
                j=temp[0]
                i=temp[1]
                print(str(j)+" "+str(i)+" "+str(mp[j][i].num))
            else:
                mp[j][i].number=mp[temp[0]][temp[1]].num
...
def startMine():
    ...
    pyautogui.moveTo(0,posy+2,0.001)

def main():


    thread=threading.Thread(target=startMine())
    thread.daemon=True
    thread.start()
    ...
    l1font=font.Font(size=14)
    label1 = tk.Label(root,text="パーツエリア",font=l1font)
    label1.place(x=0,y=75)
    
    l2font=font.Font(size=14)
    label2 = tk.Label(root,text="プログラムエリア",font=l2font)
    ...
    li[cnt]=blocks('images/mae',1,50,70,cnt,1)
    li[cnt+1]=blocks('images/migi',1,50,200,cnt+1,2)
    #li[cnt+2]=blocks('images/end',0,50,460,cnt+2,3)
    li[cnt+2]=blocks('images/hidari',1,50,330,cnt+2,3)
    li[cnt+3]=blocks('images/loopU',1,50,460,cnt+3,4)
    li[cnt+4]=blocks('images/loopD',0,50,590,cnt+4,5)
    #cnt+=24
    cnt+=5
    ...
    img=Image.open('images/start2.png')
    img=ImageTk.PhotoImage(img)

    img2=Image.open('images/connect.png')
    img2=ImageTk.PhotoImage(img2)

    #Button1 = tk.Button(text=u'スタート',height=10,width=20,command=start)
    Button1 = tk.Button(image=img,command=start)
    
    Button2 = tk.Button(image=img2,command=connect)
    ...
    createB = tk.Button(text=u'レベル構築' ,font=("",20),width=20,height=5,command=lambda:applist())
    #w=50,h=10
